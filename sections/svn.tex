\begin{frame}{Git SVN}{Cloning}
  \begin{itemize}
    ...
    \comm{git svn clone -s \dhyphen prefix=svn/ -A authors.txt -r14542:HEAD https://10.32.93.102:9880/svn/pegaplan}
  ...
  \comm{git checkout -b 9.0 remotes/svn/9.0}
  ...
  \comm{git svn dcommit \dhyphen username="ehszandi"}
  \end{itemize}
\end{frame}
...
\comm{git ch -b NEW\_BRANCH remotes/svn/NEW\_BRANCH}
  \item After updating the local repository, push it into the remote branch
  \comm{git svn dcommit \dhyphen username="ehszandi"}
\item Create a remote branch 
  \comm{git svn branch \dhyphen destination branches NEW\_BRANCH}
%\item Deleting a remote branch 
%  \comm{git push -d svn/BRANCH\_FOR\_DELETION} 
  \end{itemize}
\end{frame}
